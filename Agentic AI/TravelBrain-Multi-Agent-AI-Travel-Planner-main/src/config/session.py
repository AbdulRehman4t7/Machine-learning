...
def require(*names: str) -> None:
    ...
    resolvers = {
        ...
        # DATABASE_URL is optional — MemorySaver is used when absent
        "DATABASE_URL": resolve_database_url,
    }

    # Only check resolvers that actually exist (skip DATABASE_URL silently)
    missing = [name for name in names if name in resolvers and not resolvers[name]()]
...
CREDENTIAL_LABELS = {
    "GROQ_API_KEY": {
        "label": "Groq API key",
        "hint": "Powers every agent. Create one free at console.groq.com/keys.",
        "placeholder": "gsk_...",
    },
    # DATABASE_URL removed — MemorySaver is used for local dev (no DB needed)
}
...
def credential_status() -> dict:
    ...
    sources = {
        ...
        # DATABASE_URL is intentionally omitted — MemorySaver handles persistence
    }
